...
using org::segames::library::math::Dimension2i;
	...
	namespace segames
	{
		...
		namespace library
		{
			...
			namespace gl
			{
				...
				GLAbstractFramebuffer::GLAbstractFramebuffer(GLTexture* color, GLTexture* depth, const math::Dimension2i& size) :
					m_resetTexture(true),
					m_id(0),
					m_size(size),
					m_color(color),
				...
				const math::Dimension2i& GLAbstractFramebuffer::getSize() const
				{
					return m_size;
				}
				...
				void GLAbstractFramebuffer::setSize(const math::Dimension2i& size)
				{
					m_size = size;
					if (m_color)
						m_color->setSize(Dimension3i(m_size.getWidth(), m_size.getHeight(), m_color->getSize().getDepth()));
